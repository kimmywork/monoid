...
Proof.
  ...
  assert (H0: m + (n + p) = m + n + p). { rewrite add_assoc. reflexivity. }
  assert (H1: m + n = n + m). { rewrite add_comm. reflexivity. }
  rewrite H.
  rewrite H0.
  rewrite H1.
  reflexivity.
...
Theorem add_swap: forall n m p : nat,
  n + (m + p) = m + (n + p).
Proof.
  intros.
  rewrite add_assoc.
...
Proof.
  ...
  - induction n.
    + simpl. rewrite IHm. reflexivity.
    + simpl. rewrite IHm. rewrite <- IHn. simpl.
      rewrite IHm. rewrite add_swap. reflexivity.
...
Definition subnormalize (b0: bin) : bin :=
  match b0 with
  | Z => Z
  | _ => B0 b0
  end.
...
Fixpoint normalize (b0: bin) : bin :=
  match b0 with
  | Z => Z
  | B0 b1 => subnormalize (normalize b1)
...
Lemma incr_nat_to_bin_plus__BS_nat_to_bin : forall n,
  incr (nat_to_bin (n + n)) = B1 (nat_to_bin n).
Proof.
  induction n.
  - reflexivity.
  - rewrite <- plus_n_Sm.
