...
statement:
    | e=expr SEMICOLON { SimpleStatement e }
    ;
...
binding_pattern:
    ...
    | LPAREN binding=binding_pattern RPAREN { binding }
    | LPAREN
        bindings=separated_nonempty_list(COMMA, binding_pattern)
        RPAREN { DestructTupleBinding (None, bindings) }
    | typ=id LPAREN
        bindings=separated_list(COMMA, binding_pattern)
        RPAREN { DestructTupleBinding (Some typ, bindings) }
    | LSQUARE
        bindings=separated_list(COMMA, binding_pattern)
        RSQUARE { ListBinding (bindings) }
    | SPREAD id=option(id) { SpreadBinding id }
    | LCURLY bindings=separated_list(COMMA, named_binding) RCURLY { ObjectBinding bindings }
    | WILDCARD { WildcardBinding }
    ;

named_binding:
    | id=id COLON pattern=binding_pattern { (id, pattern) }
    | SPREAD id=id { (id, SpreadBinding (Some id)) }
    ;
...
if_expr:
    | KEYWORD_IF condition=expr
        ...
        KEYWORD_ELSE alternative=expr { IfElseExpr (condition, consequence, alternative) }
    ;
